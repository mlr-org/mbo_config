...
packages = c("data.table", "mlr3", "mlr3learners", "mlr3misc", "mlr3mbo", "mlr3pipelines", "bbotk", "paradox", "ranger", "R6", "checkmate", "mlr3extralearners")
...
registry_name = "/glade/derecho/scratch/marcbecker/botorch_pure_numeric_ncar"
...
search_space = ps(
  ...
  # surrogate
  surrogate              = p_fct(c("rf", "gp", "botorch")),
...
addAlgorithm(
  ...
  fun = function(
    ...
    file = file(sprintf("botorch/logs/pure_numeric/%i_%s.log", id, job$prob.name), open = "wt")
    sink(file)
    ...
    } else if (surrogate == "botorch") {
      learner = lrn("regr.botorch_singletaskgp", predict_type = "se")
    }
...
gp_config = data.table(
  ...
  acqopt = "LS",
  id = 1L,
  config_hash = uuid::UUIDgenerate(n = 1L)
)

botorch_config = data.table(
  input_trafo = "none",
  output_trafo = "none",
  init = "random",
  init_size_fraction = "0.25",
...
init = rbind(gp_config, botorch_config)
...
job_ids = getJobTable()$job.id
submit_ncar(job_ids, reg, template = "pbs_derecho_main.tmpl", n_jobs = 128L, log_dir = ".")

unnest(getJobTable(), c("algo.pars", "prob.pars"))

res = reduceResultsList(seq(16), reg = reg, missing.val = NA)
