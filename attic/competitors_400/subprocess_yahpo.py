...
def main():
    ...
    parser.add_argument(
        "--input_path", required=True, help="Path to JSON file with input configuration"
    )
    ...
    bench = benchmark_set.BenchmarkSet(
        args.scenario, instance=args.instance, multithread=False
    )
    ...
    y = bench.objective_function(
        X, seed=int(args.seed), logging=False, multithread=False
    )[0]
