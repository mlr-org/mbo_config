...
registry_name = "/glade/derecho/scratch/marcbecker/yahpo_mixed_deps_coordinate_descent_runtime_ncar"
...
addAlgorithm(
  ...
  fun = function(
    ...
    file = file(sprintf("coordinate_descent/logs/mixed_deps/%i_%s.log", id, job$prob.name), open = "wt")
    sink(file)
...
xdt = get_generation(list(search_space = search_space), init)
xdt$parameter = NULL
set(xdt, j = "id", value = seq_len(nrow(xdt)))
set(xdt, j = "config_hash", value = uuid::UUIDgenerate(n = nrow(xdt)))

addExperiments(
  algo.designs = list(mbo = xdt),
  repls = 1L,
  reg = reg
)
...
unnest(getJobTable(reg = reg), c("algo.pars", "prob.pars"))
