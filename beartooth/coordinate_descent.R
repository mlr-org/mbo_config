...
source("beartooth/OptimizerCoordinateDescent.R")
...
reg = makeExperimentRegistry(
  ...
  conf.file = "/home/mbecke16/mbo_config/beartooth/batchtools.conf.R",
)
...
addAlgorithm(
  ...
  fun = function(
    ...
    # seed_file = file.path(job$external.dir, "seed")
    # if (file.exists(seed_file)) {
    #   seed = as.integer(readLines(seed_file, n = 1)) + 1L
    #   set.seed(seed)
    #   writeLines(as.character(seed), seed_file)
    # } else {
    #   set.seed(1)
    #   writeLines("1", seed_file)
    # }
...
objective = ObjectiveRFunDt$new(
  fun = function(xdt, reg, fs_average, fs_extrapolation) {
    ...
    # # retry failed jobs with new seed
    # while(TRUE) {
    #   expired = findExpired(reg = reg)
    #   errors = findErrors(reg = reg)

    #   if (nrow(expired) || nrow(errors)) {
    #     message("Jobs failed!")
    #     retry_job_ids = c(expired$job.id, errors$job.id)
    #     message(getJobTable(reg = reg)[job.id %in% retry_job_ids, error])

    #     submitJobs(ids = retry_job_ids, reg = reg)

    #     waitForJobs(ids = retry_job_ids, reg = reg)
    #   } else {
