...
# per task RSNS
pdf("competitors/results/mixed_deps_mean_meta_score_per_task.pdf", width = 10, height = 10)
ggplot(data_aggr, aes(x = iter, y = meta_score, color = algorithm, fill = algorithm)) +
  ...
  ylim(-1, 1.5) +
  labs(x = "Iteration", y = "RSNS", color = "Algorithm", fill = "Algorithm") +
  scale_color_manual(values = algo_colors) +
  scale_fill_manual(values = algo_colors) +
  theme_minimal()
dev.off()
...
# per task RSNS
pdf("competitors/results/paper/mixed_deps_mean_meta_score_per_task.pdf", width = 11.69, height = 8.27)
ggplot(data_aggr, aes(x = iter, y = meta_score, color = algorithm, fill = algorithm)) +
  geom_line() +
