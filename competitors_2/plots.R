...
walk(c("numeric", "mixed"), function(benchmark) {
  ...
  runtimes = fread(sprintf("competitors_2/results/%s_runtimes.csv", benchmark), colClasses = c("dim" = "factor"))
  ...
  ggplot(data, aes(x = fraction_budget, y = mean_meta_score, color = algorithm, fill = algorithm)) +
    geom_line() +
  ...
  ggplot(data, aes(x = fraction_budget, y = mean_rank, color = algorithm)) +
    geom_line() +
