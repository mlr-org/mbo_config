...
walk(c("numeric", "mixed"), function(benchmark) {
  ...
  rs_reference = fread(sprintf("random_search/results/%s_rs_reference.csv", benchmark), colClasses = c("instance" = "character"))
  hq_registry_dir = sprintf("/glade/derecho/scratch/marcbecker/mbo_config/registries/competitors_%s_2", benchmark)
  hq_job_table = readRDS(sprintf("%s/%s_job_table_competitors.rds", hq_registry_dir, benchmark))
  bt_registry = loadRegistry(sprintf("/glade/derecho/scratch/marcbecker/mbo_config/registries/competitors_mlr3mbo_%s_2", benchmark))
  ...
  fwrite(runtimes, file = sprintf("competitors_2/results/%s_runtimes.csv", benchmark))

  # save archive
  results = rbindlist(list(results_competitors, results_mlr3mbo), use.names = TRUE)
  fwrite(results, sprintf("competitors_2/results/%s_archive.csv", benchmark))
  ...
  fwrite(aggr_results, sprintf("competitors_2/results/%s_aggr.csv", benchmark))
  ...
  fwrite(data, sprintf("competitors_2/results/%s_result.csv", benchmark))
})
