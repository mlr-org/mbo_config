...
write.xlsx(data_paper, file = "coordinate_descent/results/archive_mixed.xlsx", sheetName = "archive_mixed", col.names = TRUE, append = FALSE)
...
# best configs mixed deps
data = fread("coordinate_descent/results/archive_mixed_deps.csv")
data_2 = melt(
  data,
  id.vars = "mean_meta_score",
  measure.vars = c("input_trafo", "output_trafo", "init", "init_size_fraction", "random_interleave_iter", "trees", "variance_estimator", "acqf", "lambda", "acqopt", "epsilon_decay", "lambda_decay"),
...
fwrite(data_2, "coordinate_descent/results/best_scores_mixed_deps.csv")
