...
numeric_objective = function(
  ...
  reticulate::use_condaenv("yahpo_gym", required = TRUE) # /glade/work/marcbecker/conda-envs/yahpo_gym
  ...
  } else if (surrogate == "gam") {
    rhs = c(sprintf("s(%s, k=10)", search_space$ids()))
    formula = reformulate(rhs, response = target_variable)
    learner = LearnerRegrGam$new()
    learner$configure(predict_type = "se", formula = formula)
  }
...
if (FALSE) {
  ...
  source("gam/learner_mgcv_regr_gam.R")
  source("gam/numeric_objective.R")
