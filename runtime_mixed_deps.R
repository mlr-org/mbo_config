...
x[, list(mean_runtime = mean(time.running), min_runtime = min(time.running), max_runtime = max(time.running)), by = c("surrogate", "acqopt")][order(max_runtime, decreasing = TRUE, na.last = FALSE)]
