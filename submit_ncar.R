...
submit_ncar = function(job_ids, reg, template, n_jobs = 128) {
  ...
  walk(chunks, function(chunk) {
    ...
    hash = sprintf("%s_%s_%s", time, chunk[1], chunk[length(chunk)])

    assign("job.name", sprintf("job_%s", hash), env = env)
    assign("log.file", sprintf("log_nodes/job_%s.log", hash), env = env)
    ...
    if (length(chunk) < n_jobs) {
      walk(seq(length(chunk) + 1, n_jobs), function(i)  assign(sprintf("uri_%i", i), "", env = env))
    }
