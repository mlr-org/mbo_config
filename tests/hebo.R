...
unlink(file.path(root, "hebo_competitors"), recursive = TRUE)
reg = makeExperimentRegistry("hebo_competitors",
  conf.file = NA,
  packages = packages,
  source = source_files)
saveRegistry(reg)
...
optimizers = data.table(algorithm = c("hebo")) # "smac4bb", "hebo", "ax"
